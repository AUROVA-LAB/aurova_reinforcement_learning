...
from source.isaaclab_tasks.isaaclab_tasks.manager_based.aurova_reinforcement_learning.rl_manipulation.train.networks import CustomActorCriticPolicy
...
def receive_tensor(conn):
    ...
    tensor_bytes = recv_all(conn, size)
    buffer = io.BytesIO(tensor_bytes)
    return torch.load(buffer)


def send_tensor(s, tensor, ):
    # serialize tensor
    buffer = io.BytesIO()
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: dict):
    ...
    if args_cli.train:
        ...
        # model = PPO.load(os.path.join(teacher_path, "2026-03-30_09-29-03/model.zip")) # --> SE3 PP
        # model = PPO.load(os.path.join(teacher_path, "2026-03-30_13-19-27/model.zip")) # --> TQ PP
        # model = PPO.load(os.path.join(teacher_path, "2026-03-31_16-28-35/model.zip")) # --> GRAM ABSOLUTE PP
        # model = PPO.load(os.path.join(teacher_path, "2026-03-27_14-57-44/model.zip")) # --> GRAM REL PP
        model.policy.eval()
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            with conn:
                ...
                while True:
                    ...
                    tensor_grip = tensor[:, :-6]
                    tensor_traj = tensor[:, -6:]

                    print(tensor_grip.shape)
                    print(tensor_traj.shape)
                        
                    tensor_grip = torch.tensor(model.predict(tensor_grip)[0])
                    tensor_traj = torch.tensor(model_traj.predict(tensor_traj)[0])

                    tensor_send = torch.cat((tensor_traj, tensor_grip), dim = -1)

                    print("Tensor model: ", tensor_send)
                    send_tensor(conn, tensor_send)
